...
def get_omega_aveTime():
    ...
    for i in range(len(yearS)):
        ...
        for k in range(yearS[i],yearE[i]+1):
            ...
            prec_yMax_idx = np.moveaxis(np.ones([latlen,lonlen,360]) * range(360), -1, 0) == prec_yMax_idx
...
def month2daily(month_data,ys,ye):
    ...
    days       = [30,30,30,30,30,30,30,30,30,30,30,30] * (ye - ys + 1)
    latlen     = month_data.shape[1]
    lonlen     = month_data.shape[2]
    dayly_data = np.zeros(((ye -ys +1)*360,latlen,lonlen))
    iter_time  = 0
...
def getFilepath1(var,src_id,exp_id,var_lbl,grid_lbl,ys,ye):
    filePath = 'rawData/forcings/' + var_name[var]+'_'+freq[var]+'_'+src_id+'_'+ exp_id+'_'+ var_lbl+'_'+grid_lbl+'_'+str(ys)+'0101-'+str(ye)+'1230.nc'
    return filePath
...
def extrt_yMax(varArray):
    nyear = int(varArray.shape[0] / 360)
    varArray_yMax = np.zeros((nyear,varArray.shape[1],varArray.shape[2]))
    for i in range(nyear):
        varArray_yMax[i,:,:] = varArray[360*i:360*(i+1),:,:].max(axis = 0)
    return varArray_yMax

def getFilepath_pr(src_id,exp_id,var_lbl):
    filePath = 'rawData/forcings/pr_day_' + src_id + '_' + exp_id + '_' + var_lbl + '_gn_19500101-20141230.nc'
    return filePath

def getFilepath_tas(src_id,exp_id,var_lbl):
    filePath = 'rawData/forcings/tas_day_' + src_id + '_' + exp_id + '_' + var_lbl + '_gn_19500101-20141230.nc'
    return filePath
